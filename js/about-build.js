...
(function () {
  ...
  const aboutCover = about.closest('.about-cover');
  const skels    = qa('.skel');             // DOM order: eyebrow, quote, para1, para2, cta
  const illoWire = q('.illo-wire');
  // Queried lazily inside run() (NOT here) so the build always animates the
  // CURRENTLY live nodes. The i18n layer swaps .about-quote's innerHTML when the
  // language changes, which detaches the original .aq-line nodes — capturing them
  // here would leave the build animating stale, orphaned nodes and the (Spanish)
  // header would never appear. Re-querying at run time fixes EN and ES alike.
  let eyebrow, qlines, paras, cta, img;
  ...
  if (aboutCover) aboutCover.classList.add('is-wiring');
  ...
  function run() {
    // Grab the live, current-language nodes now (see note above).
    eyebrow = q('.about-eyebrow');
    qlines  = qa('.about-quote .aq-line');
    paras   = qa('.about-body p');
    cta     = q('.about-cta');
    img     = q('.about-illo img');
    ...
    /* the cover backstop behind the film's rounded corners returns in lockstep */
    if (aboutCover) track(aboutCover.animate(
      [{ backgroundColor: BLACK }, { backgroundColor: ICY }],
      { duration: 880, delay: 900, fill: 'both', easing: 'cubic-bezier(.4,0,.3,1)' }
    ));

    /* 2 — lock into place + unblur into focus (geometric, staggered top→down) */
    // The eyebrow is pure dark text; keep its focus-in spanning the black→icy
    // palette return (ends ~1780) so it visibly resolves AS the background
    // lightens, instead of finishing against black and only "appearing" once the
    // bg catches up — which read as the header popping in late.
    focusIn(eyebrow, 1120, 700);
    ...
    const skelFade = [1180, 1040, 1340, 1480, 1660];
    ...
    window.setTimeout(() => {
      ...
      if (aboutCover) aboutCover.classList.remove('is-wiring');
