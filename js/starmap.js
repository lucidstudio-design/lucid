...
(function () {
  ...
  function frame(now) {
    ...
    const zc = zoomComp();
    const cx = w / 2, cy = h / 2;
    // Scale is HEIGHT-dominant + a mild width term, so a narrower browser keeps
    // the field's on-screen density and just CROPS at the sides (never saturates,
    // never empties). The coverage term guarantees the field still reaches the
    // corners on very wide / heavily zoomed-out viewports, where the height term
    // alone would leave empty bands at the left & right edges. Stateless, so
    // zooming out then back in is perfectly proportional. Full-desktop unchanged.
    const heightTerm = h * 1.18 + Math.min(w, h * 2) * 0.16;
    const coverTerm = Math.hypot(w, h) * 0.52;
    const scale = Math.max(heightTerm, coverTerm);
